...
for i in range(number_of_files // 2):
	command = "dd if=/dev/zero of=./sourcedir/file_%d bs=1 count=%d > /dev/null 2>&1" % (i, each_file_size)
	subprocess.check_output(command, shell=True)
for i in range(number_of_files // 2, number_of_files):
	command = "dd if=/dev/zero of=./sourcedir/level2dir/file_%d bs=1 count=%d < /dev/null 2>&1" % (i, each_file_size)
	subprocess.check_output(command, shell=True)
